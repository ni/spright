...
namespace SprightBlazorArmstrong.Tests.Acceptance
{
    ...
    public abstract class AcceptanceTestsBase : IClassFixture<BlazorServerWebHostFixture>
    {
        ...
        protected async Task<AsyncDisposablePage> NewPageForRouteAsync(string route)
        {
            ...
            await WaitForSprightBlazorArmstrongInitializationAsync(page);
        ...
        private async Task WaitForSprightBlazorArmstrongInitializationAsync(IPage page)
        {
            await page.WaitForFunctionAsync("window.SprightBlazorArmstrong && window.SprightBlazorArmstrong.calledAfterStarted === true");
        }
